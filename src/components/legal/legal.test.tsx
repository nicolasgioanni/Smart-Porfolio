...
describe("legal document routes", () => {
  ...
  it("discloses active contact processing, providers, retention, theme storage, and the correction channel", () => {
    ...
    expect(screen.getByText(/Preparing and running Turnstile during the final Send action/)).toBeInTheDocument();
    expect(screen.getByText(/interaction-only widget remains hidden unless Cloudflare requires visitor input/)).toBeInTheDocument();
    expect(screen.getByText(/fresh, single-use token for the new message/)).toBeInTheDocument();
