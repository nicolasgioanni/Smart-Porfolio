...
describe("legal document routes", () => {
  ...
  it("discloses active contact processing, providers, retention, theme storage, and the correction channel", () => {
    ...
    expect(screen.getByText(/first name, last name, email address, optional phone number, a message of up to 500/)).toBeInTheDocument();
    expect(screen.getByText(/Opening the form records a form-start timestamp/)).toBeInTheDocument();
    expect(screen.getByText(/visible upfront Turnstile gate.*before contact fields are entered/)).toBeInTheDocument();
    expect(screen.getByText(/message of up to 500 characters.*both required acknowledgments/)).toBeInTheDocument();
    expect(screen.getByText(/fresh, single-use token/)).toBeInTheDocument();
    expect(screen.getByText(/requires successful verification, the expected action and hostname, and matching custom data/)).toBeInTheDocument();
    ...
    expect(screen.getByText(/uncertain delivery outcome, reviewed fields are locked/)).toBeInTheDocument();
    expect(screen.getByText(/still-valid signed ticket permits only that same locked delivery retry/)).toBeInTheDocument();
    expect(screen.getByText(/returns to the locked review and does not send automatically/)).toBeInTheDocument();
    expect(screen.getByText(/stores a keyed HMAC-SHA-256 value rather than the address itself/)).toBeInTheDocument();
    expect(screen.getByText(/changed same-identifier retry is rejected without storing the field values/)).toBeInTheDocument();
  ...
  it("states the scoped contact architecture, active safeguards, and disclosure limits", () => {
    ...
    expect(screen.getByText(/two required acknowledgments.*500-character message maximum/)).toBeInTheDocument();
    expect(screen.getByText(/server verifies the token through Cloudflare Siteverify/)).toBeInTheDocument();
    expect(screen.getByText(/custom data matching the submitted identifier/)).toBeInTheDocument();
    expect(screen.getByText(/operation UUID scopes one bounded retry/)).toBeInTheDocument();
    ...
    expect(screen.getByText(/time spent at the gate does not satisfy the minimum completion check/)).toBeInTheDocument();
    ...
    expect(screen.getByText(/returns to locked review without sending automatically/)).toBeInTheDocument();
    ...
    expect(screen.getByText(/opaque keyed fingerprint of the normalized full delivery payload/)).toBeInTheDocument();
