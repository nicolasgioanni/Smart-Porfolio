...
Run documentation validation, lint, type checking, focused tests, and the relevant build before handoff. Use browser tests for focus, scroll, responsive geometry, fixed positioning, rendering, and painted transitions. Keep deterministic runners bounded when external emulation or browser timing needs isolation; fix the scheduling source rather than weakening assertions or raising timeouts without evidence.
