...
The Research route gives the three established projects concise display titles, then uses `home_title` with formal `title` fallback for other IDs. It displays the authored organization logo and alt text beside a subtle order label, a distinct formal title when appropriate, impact, every published link, and deduplicated pending-link labels; it intentionally omits role, organization text, date range, and location. CytoCV, Adversarial Machine Learning, and Guide Donor Scheduler receive verified project-specific Overview and Technical narratives plus scientific SVG diagrams. Unknown research IDs fall back to `home_summary`, `detail_summary`, bullets, and skills. Content order controls the left-right visual alternation. This schema-only stage does not yet render `graphical_abstract`, `graphical_abstract_alt`, or `video`; legacy `image` is likewise not rendered.
