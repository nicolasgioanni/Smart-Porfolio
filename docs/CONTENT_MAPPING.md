...
Research graphical-abstract fields are validated and rendered inputs. The path and alternative text are paired, and the optional video depends on that abstract as its poster. Video remains validated but is not rendered until the separate self-hosted-video module is introduced. Existing `links` and `pending_links` continue to own live-site, source, DOI, manuscript, and later publication destinations.
