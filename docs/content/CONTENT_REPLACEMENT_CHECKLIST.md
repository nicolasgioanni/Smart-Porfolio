...
- [ ] Set and verify `legal_contact_email`, the Security notice `legal_effective_date`, `hosting_provider_name`, and the provider's HTTPS `hosting_privacy_url`; review the component-owned Terms, Contact Terms, and Privacy Notice dates whenever their disclosure copy changes.
- [ ] Keep the footer-only Contact form link pointed at `/contact`, and keep the public direct-email link available as a fallback.
- [ ] Confirm the Privacy and Security notices accurately describe the visible upfront Turnstile gate, submission custom-data binding, three data-entry steps, two acknowledgments, 500-character message limit, the `/api/contact/verify` ticket endpoint, the `/api/contact` delivery endpoint, the short-lived essential cookie, bounded mail-domain DNS validation, the pseudonymous D1 quota and keyed retry-payload fingerprint, sequential Resend delivery, required email/optional phone fields, and provider processing.
