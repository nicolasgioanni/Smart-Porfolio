...
The Research route gives the three established projects concise display titles, then uses `home_title` with formal `title` fallback for other IDs. It displays the authored organization logo and alt text beside a subtle order label, a distinct formal title when appropriate, impact, every published link, and deduplicated pending-link labels; it intentionally omits role, organization text, date range, and location. CytoCV, Adversarial Machine Learning, and Guide Donor Scheduler receive verified project-specific Overview and Technical narratives plus contained graphical abstracts. A complete canonical `graphical_abstract` and `graphical_abstract_alt` pair wins. When both fields are absent, those three known IDs use their checked-in curated abstract so the legacy production workbook remains presentable; incomplete pairs fail validation and never fall back. Legacy `image` is never consumed. The exact CytoCV ID also receives the registered self-hosted supplementary video when no canonical `video` value is present, or when that value selects its exact registered path; its visual retains the graphical abstract as a contained poster and separately preserves the graphical-abstract preview. Unknown research IDs fall back to `home_summary`, `detail_summary`, bullets, skills, and a neutral labelled diagram when media is absent. Content order controls the left-right visual alternation.

Published links keep their authored labels and order. A pending label that matches a published label is suppressed case-insensitively. Other pending labels render as native disabled controls with their exact authored visible label; the CytoCV `Manuscript` entry is announced as not yet published until a URL is published.
