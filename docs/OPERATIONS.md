...
4. Confirm the workflow selected `deploy_target=preview` and deployed the current `develop` SHA.
...
5. Confirm the final branch-tip check passes before Wrangler.
6. Confirm the assigned-domain smoke step passes.
7. Verify the public custom domain, key static routes, security headers, and the contact page manually.
8. Record the active `contentHash`, `commitSha`, workflow run, and any manual checks in the release record.
...
- The Resend sending identity remains verified.
...
- Automated smoke checks do not exercise valid POST requests or downstream delivery.
- Live WAF, Cloudflare plan, DNS, TLS, custom-domain, provider-secret, and branch-protection state are external and unverified by repository tests.
