...
2. Configure a matching Managed development widget for the exact local hostname. Do not add local hosts to the production widget. The client owns `execution: "render"` and `appearance: "always"` for the visible gate.
...
The visible security gate precedes three data-entry steps for names, contact details, and review. The flow is split across two same-origin endpoints:

1. The browser renders Turnstile with a fresh submission UUID in `cData` before showing contact fields.
...
4. Successful verification enables Continue and opens the form after 500 milliseconds unless the visitor continues sooner. Opening the form records the form-start time. Review requires two acknowledgments, and the message accepts at most 500 characters.
5. Send request locks the reviewed payload and submits it with the same ID to `/api/contact`.
6. The delivery Function validates the signed ticket, checks the mail domain, and reserves one of two rolling 24-hour local D1 slots without calling Siteverify again.
7. It asks Resend to accept the visitor confirmation first and then the owner notification, using separate idempotency keys.
8. Successful delivery clears the ticket. A delivery failure retains a still-valid ticket and reservation for a safe same-ID retry. An expired ticket returns the same locked UUID, start time, and payload to the gate; successful refresh returns to locked review without auto-delivery. A new logical message always receives a fresh token and UUID, then a fresh form-start time when its form opens.
