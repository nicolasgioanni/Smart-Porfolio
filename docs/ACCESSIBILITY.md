...
CytoCV's self-hosted supplementary video uses native controls with no autoplay, metadata-only preload, inline playback support, and an enabled synchronized English captions track. The player offers a readable time-coded transcript and an MP4 download fallback outside the native control surface. Playback loading, failure, and completion are politely announced without hiding either fallback. Its enlarged view reuses `ModalDialog`, keeps native captions, fullscreen, and picture-in-picture available, and pauses and resets both views when it closes or reopens so no audio resumes unexpectedly. Review caption terminology against the source video whenever the asset changes; see [Research media](RESEARCH_MEDIA.md) for the current integrity and publication record.
