...
| Contact step transitions | `src/components/contact/useContactStepTransition.test.tsx` | Measured-height completion, replacement-animation isolation, content-resize interruption, frame removal, reduced-motion updates, and unsupported-animation fallback |
...
The contact browser specification also checks unchanged gate geometry across loading, expiry, widget shrink/removal, failure, retry, pending server confirmation, and success at desktop/mobile widths, in all themes and reduced motion. Its normal-motion geometry baseline waits for the finite route-entry animation to finish. It samples the contact card and normal-flow email fallback during measured grow and shrink transitions, checks rapid rebase, and retains reduced-motion immediate behavior. Normal-motion Back cases use the actual scrolled lower-step control at desktop and phone widths, then verify settled heading focus, viewport visibility, released clipping, and preserved values. It also covers stacked notification peeks, touch/keyboard expansion and dismissal, focus recovery, viewport scrolling, email suffix typos, and retained completion/recovery. Unit tests separately control independent 30-second clocks, duplicate refresh, capacity eviction, announcements, and bounded step-transition cleanup. Existing skeleton baselines and all three skeleton regression gates remain unchanged.

## Interface and accessibility coverage

| Area | Test files | Main contracts |
| --- | --- | --- |
| Navigation and links | Tests under `src/components/navigation/`, plus responsive-query and theme tests | Route registry and `SmartLink` internal-route recognition, active indicator, fixed mobile dock, unified route-and-action rail order, overflow state, idle motion, five-second interaction pause and resume, responsive transitions, external-link safety, and cleanup |
...
| Research showcase | `src/components/portfolio/research/ResearchShowcase.test.tsx`, `src/components/portfolio/research/ResearchExplainer.test.tsx`, `src/components/portfolio/research/researchStyles.test.ts`, `src/lib/content/researchGraphicalAbstracts.test.ts`, `src/lib/content/researchVideos.test.ts` | Project order, canonical-over-curated media resolution, legacy-workbook-safe fallbacks, centered two-row media columns, strict local video resolution, controlled AML and GuideDonorScheduler connected scenes, custom-player layout, responsive behavior, resource states, audience depth, disclosure isolation, motion, and Escape behavior |
| Research video assets | `src/lib/media/researchVideoAssets.test.ts`, `src/lib/media/researchVideoPlayback.test.ts`, `src/components/portfolio/research/ResearchVideoPreview.test.tsx` | Immutable MP4, LF-only captions, and visual-description transcript hashes; bounded deterministic MP4 stream metadata and WebVTT timing; native no-JavaScript controls; enhanced transport, captions, transcript fallback, metadata/error recovery, and paused timeline/settings handoff without automatic playback through the shared fitted dialog |
...
| Shared dialogs | `src/components/overlay/ModalDialog.test.tsx`, `src/components/navigation/navigation.test.tsx`, `src/components/portfolio/skills/PortfolioSkillShowcase.test.tsx`, `src/components/portfolio/projects/ProjectSkillShowcase.test.tsx`, `src/components/portfolio/research/ResearchGraphicalAbstractPreview.test.tsx` | Portal lifecycle, accessible names, informative abstract alt text, initial focus, native media fallback, consumed Escape, fullscreen focus scope, hidden/inert control filtering, focus containment and recovery through exit, topmost stack isolation, Escape, backdrop isolation, exact scroll-lock restoration, reduced motion, rapid reopen, and trigger-focus restoration |
...
`tests/e2e/experience.spec.ts` protects the shared detail-level control on the Experience route: it verifies the overview-to-technical switch, 44 CSS-pixel mode-button targets, scoped live status, route-wide disclosure exclusivity, Escape and outside dismissal, compact desktop control placement, small-screen flow, breakpoints, palette surfaces, and reduced-motion transitions. It tabs through the named desktop panel body, verifies keyboard scrolling and Escape focus restoration, then reaches a covered next summary with the keyboard. Frame samples cover opening, closing, direct switching, and rapid reopen while summary rows, resources, and following cards remain fixed above `980px`; at `980px` the same content grows in natural flow. Its connected-surface checks use each palette and the glass-effects fallback to confirm the final evidence row keeps a seam-free opaque summary/panel connection, painted outline, keyboard focus ring, and close-state geometry. It derives cards and disclosures from the rendered content and asserts the explicit empty state when the valid workbook has no experience entries. When an expandable card exists, its focused-scroll regression opens a disclosure, moves the pointer away without blurring the control, proves downward and upward scrolling, and confirms the expanded card retains its resting transform and shadow in My mode, Light, and Dark.

`tests/e2e/research.spec.ts` applies the same browser-level detail contracts, including 44 CSS-pixel mode-button and resource targets, to the Research redesign while checking its alternating visual/content rows. Pull-request runs generate the checked-in template content before the suite executes; main and develop pushes use the validated live workbook snapshot instead. The browser test discovers rendered cards and expandable disclosures rather than relying on template IDs, titles, prose, or a fixed project count. It asserts the explicit empty state when no projects render; otherwise it checks order labels, authored organization-logo accessibility, unavailable-resource semantics, route-wide overlay selection, Escape and outside dismissal, named keyboard-scrollable panel bodies, desktop alternation, outer visual-column clipping, narrow-screen stacking without horizontal overflow, palette surfaces, reduced-motion fallbacks, and the same all-palette focused-scroll elevation contract when the rendered content provides an expandable project. Its shared connected-surface checks cover the final evidence row in every palette and the glass-effects fallback, including the open-to-closing connection and the `981/980` overlay boundary after the panel transition settles. Frame samples preserve normal-flow rows, resources, and later projects throughout every desktop panel direction, direct switch, and rapid reopen, while the `981/980` boundary switches only the panel from absolute layering to normal flow. Graphical-abstract coverage discovers every available preview, verifies intrinsic loading and uncropped containment, Enter and Space activation, Tab and Shift+Tab focus containment, close/Escape/backdrop dismissal, exact focus and scroll-lock restoration, rapid reopen, solid surfaces in every palette, `921/920` and `621/620` breakpoint behavior, and viewport fit at `1280x600` and `320x568`. The CytoCV video coverage verifies enhanced controls, real browser playback, captions, hover/leave fades, permanent center-control geometry, split action-group placement, inset and constrained enlarged/fullscreen bars, keyboard and touch access, an expanding inline volume range, hierarchical speed settings, nested Escape, fullscreen, action containment, and no document overflow. Component server-rendering assertions protect native controls and captions before hydration. Validate the complete no-JavaScript page against the built static export; Next.js can export streamed loading boundaries that need JavaScript; the shell’s Research-only `noscript` fallback must expose one visible Research heading and native player outside those boundaries. Check native keyboard playback against a static server with media range support. Media dialogs retain measurable outside gutters and the 84dvh frame limit across short desktop and mobile sizes; their frames remain solid in Navy, Light, and Dark. Deterministic component tests cover paused inline-to-modal and modal-to-inline timeline/settings handoffs and verify no automatic `play()` calls. Research skeleton alignment checks media viewport footprints rather than equating inert loading placeholders with the enhanced toolbar action count; reviewed Linux skeleton images remain unchanged. It also covers both diagram variants through rendered-media discovery: connected scenes, persistent readable HTML labels at `320px` in every palette, captions, 44px Pause/Resume controls, four three-second phases in a twelve-second loop, actual subject motion, viewport and hidden-document suspension, manual-pause retention, and static reduced-motion output. `ResearchExplainer` server-rendering tests keep both variants complete without playback controls in static output. Both commands run in Chromium after the shared browser installation step in CI.
