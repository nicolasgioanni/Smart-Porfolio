...
| Research showcase | `src/components/portfolio/ResearchShowcase.test.tsx`, `src/components/portfolio/researchStyles.test.ts`, `src/lib/content/researchGraphicalAbstracts.test.ts`, `src/lib/content/researchVideos.test.ts` | Project order, canonical-over-curated media resolution, legacy-workbook-safe fallbacks, alternating contained abstracts, strict local video resolution, responsive layout, resource states, audience depth, disclosure isolation, motion, and Escape behavior |
| Research video assets | `src/lib/media/researchVideoAssets.test.ts`, `src/lib/media/researchVideoPlayback.test.ts`, `src/components/portfolio/ResearchVideoPreview.test.tsx` | Immutable MP4, captions, and transcript hashes; deterministic MP4 stream metadata and WebVTT timing; native controls; error/download/transcript fallbacks; reset-on-close and rapid-reopen behavior through the shared dialog |
...
| Shared dialogs | `src/components/overlay/ModalDialog.test.tsx`, `src/components/navigation/navigation.test.tsx`, `src/components/portfolio/PortfolioSkillShowcase.test.tsx`, `src/components/portfolio/ProjectSkillShowcase.test.tsx`, `src/components/portfolio/ResearchGraphicalAbstractPreview.test.tsx` | Portal lifecycle, accessible names, informative abstract alt text, initial focus, native media controls, focus containment and recovery through exit, topmost stack isolation, Escape, backdrop isolation, exact scroll-lock restoration, reduced motion, rapid reopen, and trigger-focus restoration |
