...
| Research showcase | `src/components/portfolio/ResearchShowcase.test.tsx`, `src/components/portfolio/researchStyles.test.ts`, `src/lib/content/researchGraphicalAbstracts.test.ts` | Project order, canonical-over-curated media resolution, legacy-workbook-safe fallbacks, alternating contained abstracts, responsive layout, resource states, audience depth, disclosure isolation, motion, and Escape behavior |
