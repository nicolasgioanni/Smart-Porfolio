...
The suite verifies the fixed bottom dock at 320, 390, and 768 CSS pixels; canonical route links followed by social and theme actions in one moving rail; native overflow and edge fades; viewport-bottom persistence while the document scrolls; upward unclipped theme placement; document-level overflow protection; initial idle drift; five-second interaction pause and resume from the current position; and the reduced-motion fallback. It also verifies dark and light device preference changes, manual My mode precedence, and returning to System without a stored override. A desktop case protects the sticky top header, identity, route list, and compact-on-scroll behavior. Route-level cases verify that every registered resolved page receives the body entrance while the shared header and footer do not, that same-document client navigation mounts a fresh animated page root, and that reduced motion removes the entrance. Unit and style-contract coverage keeps route skeleton roots separate from the resolved page selector.
