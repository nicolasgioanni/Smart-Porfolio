...
- closes on unconsumed Escape, backdrop activation, or the close button; nested controls can consume Escape before the dialog;
...
Each expandable evidence row is a native button with `aria-expanded` and `aria-controls`; its panel is a labelled region and is `aria-hidden`, inert, and pointer-inactive while closed or closing. When it is open, the labelled detail scrollport is in the tab order. It caps only a long desktop body at `60dvh`, so a keyboard user can use Arrow or Page keys before tabbing to the next row while shorter content retains its intrinsic height. Research and Experience each keep one selected row across the full route. Escape closes the selected row and leaves focus on its trigger. A noninteractive card-header or card-body click and an outside click close it after the target control can activate; links and text selection retain their normal behavior. Above `980px`, moving keyboard focus out of the selected row also dismisses it, while mobile pointer focus never preempts a targeted click. Each graphical-abstract thumbnail is a labelled native button; its decorative thumbnail avoids a duplicate announcement, while the enlarged dialog image carries the specific scientific alternative text. The dialog opens on a visible close control and supports Escape, backdrop dismissal, focus containment, and trigger-focus restoration. Unknown projects without media keep a concise labelled fallback diagram. Research route organization marks use the authored logo alt because the simplified card header does not repeat the organization as text. An unpublished resource is a native disabled button named `&lt;label&gt; — not yet published`, never a link or a faux disabled element; its visible label stays exactly as authored.

The Adversarial Machine Learning and GuideDonorScheduler explainers are inline SVG scenes with complete captions and persistent HTML label layers. Every meaningful label remains visible at a minimum `13px` CSS size at `320px`; the visual label layer is an `aria-hidden` duplicate of the diagram’s labelled description, avoiding repeated announcements while keeping the connected workflow available whether the animation is playing, paused, reduced, or unavailable. A native control is present only when motion can run, keeps a 44 by 44 CSS-pixel target, and has the current manual action as its accessible name: `Pause` when the animation is active or only temporarily held by offscreen or hidden-document conditions, and `Resume` after a manual pause. Initial reduced-motion or unsupported-observer rendering exposes the complete static scenes and captions without a playback control. If a focused control becomes static after a live preference change, it stays as a disabled element until focus leaves so the browser does not discard focus.

### Research video

CytoCV's self-hosted supplementary video server-renders native controls with no autoplay, metadata-only preload, inline playback support, and an enabled synchronized English captions track. `ResearchVideoPlayer` progressively replaces the controls after hydration, sharing the same labelled 44px transport buttons inline and enlarged. The centered play/pause control and bottom bar reveal on pointer activity, fade after four seconds without pointer movement, and hide on pointer exit, including while paused. Keyboard-visible focus, scrubbing, open settings or volume controls, media feedback, and touch reveal keep them operable. Its action groups keep play and sound first, with captions, settings, and fullscreen at the opposite end. Smaller visible icon surfaces do not reduce their labelled hit targets.

The sound button toggles mute directly. Its adjacent volume range reveals on hover, keyboard focus, or touch activation, remains available during focus or dragging, and leaves the focus order when collapsed. Settings open to a compact root menu; its named playback-speed row enters a vertical rate submenu with an explicit Back control. Escape returns from the speed submenu to the root menu, closes the root menu next, and only then reaches fullscreen or modal dismissal. Fine-pointer menu rows may be compact while coarse-pointer rows retain 44px targets. The enlarged-view action lives in the settings root and returns dialog focus to that persistent settings trigger. Loading, playback failure, and completion are politely announced without announcing every timestamp or caption cue. A direct transcript link remains below the inline player.

Enhanced captions use browser-parsed active cues. Inline non-fullscreen presentation uses lower and raised aria-hidden visual layers, with only the appropriate layer visible during the rapid controls handoff, so the custom presentation does not add duplicate accessible text. The native track resumes visible captions for native presentation modes; no second caption parser or runtime content fetch is introduced. Inline/enlarged handoff pauses both players and preserves time, mute, volume, speed, and caption preference. Entering or leaving container fullscreen keeps the same media element and playback state. Fullscreen support is detected, with native video fullscreen or the enlarged view as fallback. Shared dialog focus stays within its active fullscreen descendant; nested settings consume Escape before fullscreen or modal dismissal. The enlarged media close control stays visible outside fullscreen. Review captions and visual descriptions against the source whenever the asset changes; see [Research media](../content/RESEARCH_MEDIA.md) for the current integrity and publication record.
...
The route begins with a visible, labelled Turnstile gate. Contact fields are not rendered until server verification succeeds. Gate loading, success, expiry, retry, and failure states remain announced through the persistent status. Loading and server-confirmation labels use a CSS-only text highlight with a readable plain-blue fallback; reduced motion keeps the label static and forced colors use system text. Retry actions restore focus to the persistent gate heading or status instead of leaving focus on a removed control. Once server verification succeeds, the internal status and retry controls are omitted while the provider’s native completion state remains centered through the 500-millisecond transition. Success enables Continue and opens the form automatically after that brief transition unless the visitor continues sooner; focus then moves into the form.

The three data-entry steps provide visible labels, required-state cues, `aria-invalid`, error descriptions, focus movement, polite status updates, alert semantics, two native acknowledgement checkboxes, and a final Send button disabled until both acknowledgements are complete. Narrow layouts keep visual button order aligned with DOM and keyboard order. Delivery states remain announced while repeated Send actions are blocked. Successful delivery replaces the wizard with a standalone completion view and a clearly named <em>Send another message</em> action that starts a fresh gate. An expired two-hour draft exposes an explicit fresh-request action that preserves the reviewed values but requires a new UUID, Turnstile check, and start time. The hidden honeypot is removed from the tab order. Moving among data-entry steps keeps one live step and its existing heading focus target. Incoming content uses a short opacity and upward entrance while its containing frame can interpolate a single measured height; no hidden duplicate controls or live-region copies are created. Reduced motion changes steps immediately, and the verification gate remains outside the animated frame.
...
The verification well reserves widget, status, and recovery rows through every security state; it uses normal 300-by-65 dimensions when at least 300 pixels are available and compact 150-by-140 dimensions below that threshold. Email suffix typos display their specific error on blur and Review. Invalid fields retain red treatment, repeatable shake, error descriptions, and first-invalid-field focus. Submission completion focuses its persistent heading.
