...
CytoCV's self-hosted supplementary video uses native controls with no autoplay, metadata-only preload, inline playback support, and an enabled synchronized English captions track. Its labelled 44px in-player toolbar exposes direct transcript, MP4 download, and enlarged-player controls through icons with hover and focus tooltips; the controls remain visibly available without hover. Playback loading, failure, and completion are politely announced without hiding either fallback; inline and modal status messages remain independent. Its enlarged view reuses `ModalDialog`, keeps native captions, fullscreen, and picture-in-picture available, retains direct transcript and download controls, keeps its close control independently visible, and pauses while handing its current timeline between the inline and enlarged views. Neither view is programmatically played. Review captions and visual descriptions against the source whenever the asset changes; see [Research media](../content/RESEARCH_MEDIA.md) for the current integrity and publication record.
