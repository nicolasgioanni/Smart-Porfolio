...
The Research video loader preserves its reviewed three static 44px blocks inside the media frame, while the two explanatory diagram loaders reserve inert 44px playback-control placeholders. They remain `aria-hidden` placeholders without links, buttons, tooltips, or playback behavior; they do not map to the enhanced player’s action count. Alignment checks compare media viewport footprints, while the reviewed Linux skeleton geometry and images remain unchanged.
