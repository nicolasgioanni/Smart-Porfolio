...
Experience and Research cards use a restrained neutral shadow and two-pixel lift only during genuine fine-pointer hover. Their selected detail panel is opaque; on desktop, its inner body caps and scrolls only content longer than `60dvh`, while shorter content keeps its intrinsic height. It closes before a changed audience level or responsive mode can carry it across layouts. When a disclosure retains focus, the card keeps its resting geometry and shadow while the focused control supplies the keyboard-visible ring; `:focus-within` may strengthen only the card border. This prevents persistent card-wide elevation from reading as an extra layer after scrolling.
