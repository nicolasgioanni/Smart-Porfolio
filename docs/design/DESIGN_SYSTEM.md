...
Each abstract keeps a solid, rounded, locally clipped visual column with a fixed 16px top-aligned inset that fills the available column width. The source abstract remains uncropped with `object-fit: contain`. Its native preview button receives a restrained direct lift on fine-pointer hover and keyboard focus without adding decorative glow or elevating the entire card. Activation opens a near-viewport dialog with a solid frame that preserves the full image in both axes and provides a visible close control. Unknown projects without a valid abstract use a neutral labelled research diagram rather than an invented project mark.
