...
Experience and Research cards use a restrained neutral shadow and two-pixel lift only during genuine fine-pointer hover. Their selected detail panel is opaque, may scroll internally after `60dvh` on desktop, and closes before a changed audience level or responsive mode can carry it across layouts. The selected summary and panel share the card's semantic opaque surface, with square joined corners, a single subtle outline, and no seam, panel shadow, or independent hover fill; that connection remains through the closing state. When a disclosure retains focus, the card keeps its resting geometry and shadow while the focused control supplies the keyboard-visible ring; `:focus-within` may strengthen only the card border. This prevents persistent card-wide elevation from reading as an extra layer after scrolling.
