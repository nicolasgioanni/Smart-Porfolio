...
| Contact delivery | Verification ticket and contact payload | Resend batch result | `/api/contact` Pages Function |
...
| Layout and navigation | `src/components/layout/`, `src/components/navigation/` | Shared shell, header, active routes, mobile menu, footer, and profile preview. |
...
| Runtime contact | `functions/` | Origin enforcement, Turnstile verification, signed tickets, schema validation, and email delivery. |
...
- active-route measurement and mobile navigation;
...
The browser submits contact fields, acknowledgements, timing metadata, honeypot value, and the same submission ID to `/api/contact`. The Function applies its request and schema rules, verifies the ticket binding, and sends an owner notification plus visitor receipt through one Resend batch request.

Successful delivery clears the ticket. Delivery failure retains an otherwise valid ticket for retry. The submission ID is also used for provider idempotency. There is no first-party contact database, and request bodies or personal fields must not be written to logs.

Cloudflare WAF rate limiting is an operator-managed prerequisite outside the repository. Source code can document and test the expected endpoint behavior, but it cannot prove the live zone rule, plan capability, or response customization. See [Contact system](CONTACT_SYSTEM.md) and [Security](SECURITY.md).
...
4. Run documentation validation, lint, typecheck, focused footer regressions, the full Vitest suite, and a static build.
...
9. Deploy the static export and Functions from repository root with pinned Wrangler.
10. Smoke-test static content, both manifests, and GET rejection from both contact Functions.
