...
The footer stays in normal document flow after every route and server-renders as a compact copyright row. A focused client wrapper observes a fixed-size trigger aligned with that row: it expands the legal and resource details when roughly half of the trigger enters the viewport, including when it is already visible after hydration, and keeps them open while the visitor scrolls downward through the details. When the visitor scrolls upward, the wrapper collapses the details once the trigger retreats through the viewport's lower edge to roughly 15% visibility. The separate open and close thresholds prevent the footer's changing height from producing observer feedback. An explicit `Details`/`Collapse` disclosure remains the device-independent fallback; manual collapse suppresses automatic reopening until the entire footer leaves the viewport and later re-enters, and focused details are never hidden. Route changes reset the disclosure before visibility is evaluated again. `/terms`, `/privacy`, and `/security` are static internal routes referenced only by the footer, not by the main header navigation.
