...
Self-hosted video, captions, and transcript paths use the same fixed-point decoded local-path guard with separate extension allowlists. `researchVideos.ts` accepts its curated fallback for the exact CytoCV project ID only when canonical content is absent, and it does not expose a video unless every companion accessibility resource passes that boundary. The video asset contract parses ISO MP4 box boundaries and expected track metadata in-process, avoiding a continuous-integration dependency on an externally installed media probe. Its byte hashes also make a silent replacement visible in review. See [Research media](RESEARCH_MEDIA.md) for the approved source statement and explicitly non-asserted license state.
