...
- Resolve self-hosted Research video only through `src/lib/content/researchVideos.ts` and `ResearchVideoPreview`. Narrated video requires a reviewed synchronized WebVTT track and readable transcript, plus a same-origin download fallback; keep native captions, fullscreen, and picture-in-picture available. Pause and reset video when its modal closes so reopening never resumes unexpectedly.
- Keep the CytoCV supplementary-video MP4, captions, and transcript byte-locked by `src/lib/media/researchVideoAssets.test.ts`; document any authorized replacement in `docs/RESEARCH_MEDIA.md` without inferring a media license from source code.
